...
public class ChessServer : MonoBehaviour
{
		...
		{	
				...
				if (!networked) {
						if (GUI.Button (new Rect (Screen.width * 3 / 4, Screen.height / 5, Screen.width / 4, Screen.height / 8), "Reset Game"))
				...
				if (!Network.isClient && !Network.isServer) {
						...
						if (showHosts) {
								string add = "playing.";
								if (!playing) 
										add = "looking for opponent.";
								if (hostList != null) {
										for (int i = 0; i < hostList.Length; i++) {
												if (GUI.Button (new Rect (Screen.width / 4, Screen.height / 8 + (Screen.height / 4 * i), Screen.width / 4, Screen.height / 8), hostList [i].gameName + this.GetComponent<ChessChat> ().chat_id + ". Currently " + add))
				...
				if (networked && playing) {
						...
						if (visitor) {
								...
								if (GUI.Button (new Rect (Screen.width * 3 / 4, Screen.height * 3 / 5, Screen.width / 4, Screen.height / 8), "Kick opponent out")) {
				...
				this.GetComponent<ChessChat> ().Draw ();
